...
class _RiderDashboardState extends State<RiderDashboard> {
  ...
  void _startLocationTracking(String orderId, OrderService orderService) async {
    ...
    // Get current location immediately so customer sees rider right away
    final initialPos = await LocationService().getCurrentLocation();
    if (initialPos != null) {
      orderService.updateRiderLocation(orderId, initialPos.latitude, initialPos.longitude);
    }
                ...
                children: [
                  ...
                  if (activeOrders.isNotEmpty)
                    Container(
                      ...
                      child: Text("${activeOrders.length} IN QUEUE", style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)),
                    ),
                ],
              ),
              const SizedBox(height: 16),
              if (activeOrders.isEmpty) 
                _buildWaitingForOrders(service, riderId) 
              else 
                _ActiveDeliveryCardWidget(orders: activeOrders, service: service, riderId: riderId),
            ],
          ),
        );
      },
...
class _ActiveDeliveryCardWidget extends StatefulWidget {
  final List<OrderModel> orders;
  final OrderService service;
  final String riderId;

  const _ActiveDeliveryCardWidget({required this.orders, required this.service, required this.riderId});

  @override
  State<_ActiveDeliveryCardWidget> createState() => _ActiveDeliveryCardWidgetState();
}

class _ActiveDeliveryCardWidgetState extends State<_ActiveDeliveryCardWidget> {
  Position? _currentPosition;
  OrderModel? _prioritizedOrder;
  ...
  void initState() {
    super.initState();
    // Track which order we're currently routing to
    _lastTrackedOrderId = widget.orders.isNotEmpty ? widget.orders.first.id : null;
  ...
  Future<void> _initMarkerIcons() async {
    ...
    final customer = await _createCustomMarker(Icons.home, Colors.red);
    if (mounted) {
      setState(() {
        _riderIcon = rider;
        _storeIcon = store;
  ...
  void _startLocationUpdates() async {
    ...
    if (await locService.checkPermission()) {
      _currentPosition = await locService.getCurrentLocation();
      _calculateQueueAndDistance();

      _locationSubscription = locService.getLocationStream().listen((Position position) {
        if (mounted) {
          setState(() {
  ...
  @override
  void didUpdateWidget(covariant _ActiveDeliveryCardWidget oldWidget) {
    super.didUpdateWidget(oldWidget);

    // If the first (prioritized) order has changed, clear the stale route cache
    // so we don't reuse directions from a completely different order.
    final newFirstId = widget.orders.isNotEmpty ? widget.orders.first.id : null;
    if (newFirstId != _lastTrackedOrderId) {
      _currentRouteData = null;
      _lastFetchedPos = null;
      _lastDestinationPos = null;
      _readableAddress = "";
      _lastTrackedOrderId = newFirstId;
    }

    _calculateQueueAndDistance();
  }

  Future<void> _calculateQueueAndDistance() async {
    
    if (!mounted) return;

    List<OrderModel> sortedOrders = List.from(widget.orders);

    if (_currentPosition != null) {
      // Sort orders by distance to their next destination
      sortedOrders.sort((a, b) {
        bool toStoreA = a.status == OrderStatus.pickingUp || a.status == OrderStatus.accepted;
        double latA = toStoreA ? (a.storeLat ?? 0) : (a.deliveryLat ?? 0);
        double lngA = toStoreA ? (a.storeLng ?? 0) : (a.deliveryLng ?? 0);
        double distA = Geolocator.distanceBetween(_currentPosition!.latitude, _currentPosition!.longitude, latA, lngA);

        bool toStoreB = b.status == OrderStatus.pickingUp || b.status == OrderStatus.accepted;
        double latB = toStoreB ? (b.storeLat ?? 0) : (b.deliveryLat ?? 0);
        double lngB = toStoreB ? (b.storeLng ?? 0) : (b.deliveryLng ?? 0);
    ...
    _prioritizedOrder = sortedOrders.isNotEmpty ? sortedOrders.first : null;

    if (_currentPosition != null && _prioritizedOrder != null) {
      bool toStore = _prioritizedOrder!.status == OrderStatus.pickingUp || _prioritizedOrder!.status == OrderStatus.accepted;
      double targetLat = toStore ? (_prioritizedOrder!.storeLat ?? 0) : (_prioritizedOrder!.deliveryLat ?? 0);
      double targetLng = toStore ? (_prioritizedOrder!.storeLng ?? 0) : (_prioritizedOrder!.deliveryLng ?? 0);
      
      if (targetLat != 0 && targetLng != 0) {
        _distanceInKm = Geolocator.distanceBetween(_currentPosition!.latitude, _currentPosition!.longitude, targetLat, targetLng) / 1000;
        
        bool destinationChanged = _lastDestinationPos == null ||
            _lastDestinationPos!.latitude != targetLat ||
            _lastDestinationPos!.longitude != targetLng;

        if (_currentRouteData == null || _lastFetchedPos == null || destinationChanged || Geolocator.distanceBetween(_currentPosition!.latitude, _currentPosition!.longitude, _lastFetchedPos!.latitude, _lastFetchedPos!.longitude) > 50) {
          ...
          if (!_isFetchingRoute) {
            ...
            if (mounted && routeData != null) {
              setState(() {
                _lastFetchedPos = LatLng(_currentPosition!.latitude, _currentPosition!.longitude);
                _lastDestinationPos = LatLng(targetLat, targetLng);
      ...
      if (!toStore && _prioritizedOrder!.deliveryAddress.startsWith("Location:")) {
        if (_readableAddress.isEmpty && targetLat != 0 && targetLng != 0) {
          try {
             List<geocoding.Placemark> placemarks = await geocoding.placemarkFromCoordinates(targetLat, targetLng);
             if (placemarks.isNotEmpty) {
               final place = placemarks.first;
               if (mounted) {
                 setState(() {
                   _readableAddress = "${place.street}, ${place.locality}";
                 });
               }
             }
          } catch(e) {}
  ...
  Future<void> _launchGoogleMapsNavigation() async {
    if (_prioritizedOrder == null) return;
    
    bool toStore = _prioritizedOrder!.status == OrderStatus.pickingUp || _prioritizedOrder!.status == OrderStatus.accepted;
    double? destLat = toStore ? _prioritizedOrder!.storeLat : _prioritizedOrder!.deliveryLat;
    double? destLng = toStore ? _prioritizedOrder!.storeLng : _prioritizedOrder!.deliveryLng;

    if (destLat == null || destLng == null || destLat == 0.0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Location coordinates missing for this order!")));
      return;
    }
    ...
    // Use standard Maps URL to ensure origin parameter is respected (google.navigation intent ignores origin)
    final Uri webUrl = Uri.parse('https://www.google.com/maps/dir/?api=1$originParam&destination=$destLat,$destLng&travelmode=driving');
  ...
  void _handleOrderAction() {
    ...
    if (_prioritizedOrder!.status == OrderStatus.accepted) {
      widget.service.updateOrderStatus(_prioritizedOrder!.id!, OrderStatus.pickingUp);
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Heading to pick up!")));
    } else if (_prioritizedOrder!.status == OrderStatus.pickingUp) {
      widget.service.updateOrderStatus(_prioritizedOrder!.id!, OrderStatus.outForDelivery);
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Package picked up! Proceed to Drop Off.")));
    } else {
      widget.service.updateOrderStatus(_prioritizedOrder!.id!, OrderStatus.delivered);
  ...
  @override
  Widget build(BuildContext context) {
    if (_prioritizedOrder == null) return const Center(child: CircularProgressIndicator());

    String distanceText = _currentRouteData != null ? _currentRouteData!.durationText : (_distanceInKm > 0 ? "${_distanceInKm.toStringAsFixed(1)} km away" : "Calculating distance...");
    String storeName = _prioritizedOrder!.storeName ?? "Vendor Store";
    bool toStore = _prioritizedOrder!.status == OrderStatus.pickingUp || _prioritizedOrder!.status == OrderStatus.accepted;
    String destName = toStore ? storeName : _prioritizedOrder!.customerName;
    String dropOffAddress = _readableAddress.isNotEmpty ? _readableAddress : _prioritizedOrder!.deliveryAddress;
    ...
    if (_currentPosition != null && _prioritizedOrder != null) {
      ...
      if (targetLat != 0 && targetLng != 0) {
        ...
        final destPos = LatLng(targetLat, targetLng);
        ...
        final Set<Marker> markers = {
          ...
          Marker(
            ...
            infoWindow: InfoWindow(title: toStore ? 'Store' : 'Customer'),
          ),
        };

        final Set<Polyline> polylines = {};
        if (_currentRouteData != null) {
          polylines.add(Polyline(polylineId: const PolylineId('route'), points: _currentRouteData!.polyline.map((p) => LatLng(p['lat']!, p['lng']!)).toList(), color: AppColors.primaryGreen, width: 5));
        } else {
           polylines.add(Polyline(polylineId: const PolylineId('route_fallback'), points: [riderPos, destPos], color: AppColors.primaryGreen.withOpacity(0.5), width: 4, patterns: [PatternItem.dash(20), PatternItem.gap(10)]));
        }
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start, 
                        children: [
                          Text(destName, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), 
                          Row(
                            children: [
                              const Icon(Icons.location_on, size: 12, color: Colors.grey), 
                              const SizedBox(width: 4), 
                              Text(distanceText, style: TextStyle(color: Colors.grey.shade600, fontSize: 12, fontWeight: FontWeight.bold))
                            ]
                          )
                        ]
                      )
                    ),
                    IconButton(icon: const Icon(Icons.navigation, color: Colors.white), onPressed: _launchGoogleMapsNavigation, style: IconButton.styleFrom(backgroundColor: Colors.blue)),
                  ],
                ),
                const SizedBox(height: 24),
                _deliveryStep(Icons.store, "PICK UP", storeName, isFirst: true, isDone: _prioritizedOrder!.status != OrderStatus.pickingUp && _prioritizedOrder!.status != OrderStatus.accepted),
                const SizedBox(height: 16),
                _deliveryStep(Icons.home, "DROP OFF", dropOffAddress, isDone: false),
              ],
            ),
          ),
          Padding(
            ...
            child: ElevatedButton.icon(
              ...
              icon: Icon(_prioritizedOrder!.status == OrderStatus.accepted 
                  ? Icons.directions_bike 
                  : (_prioritizedOrder!.status == OrderStatus.pickingUp ? Icons.check_box : Icons.check_circle)),
              label: Text(_prioritizedOrder!.status == OrderStatus.accepted 
                  ? "START PICKUP" 
                  : (_prioritizedOrder!.status == OrderStatus.pickingUp ? "MARK PICKED UP" : "MARK DELIVERED")),
  ...
  Widget _deliveryStep(IconData icon, String label, String value, {bool isFirst = false, bool isDone = false}) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Column(children: [Container(padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: isDone ? Colors.green : (isFirst ? const Color(0xFFE8F5E9) : const Color(0xFFFFF3E0)), shape: BoxShape.circle), child: Icon(icon, size: 14, color: isDone ? Colors.white : (isFirst ? Colors.green : Colors.orange))), if (isFirst) Container(width: 2, height: 20, color: Colors.grey.shade200)]),
        const SizedBox(width: 15),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(fontSize: 10, color: Colors.grey, fontWeight: FontWeight.bold, letterSpacing: 0.5)), Text(value, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: isDone ? Colors.grey : AppColors.textPrimary, decoration: isDone ? TextDecoration.lineThrough : TextDecoration.none))])),
      ],
    );
  }
}

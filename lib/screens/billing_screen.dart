...
class _BillingScreenState extends State<BillingScreen> {
  ...
  Future<void> _fetchAddress() async {
    ...
    try {
      ...
      final List addresses = doc.data()?['addresses'] ?? [];
      _userAddresses = addresses;
      if (addresses.isNotEmpty) {
        if (mounted) {
          setState(() {
            _selectedAddress = addresses[0]['full_address'];
            _selectedLat = addresses[0]['lat']?.toDouble();
            _selectedLng = addresses[0]['lng']?.toDouble();
          });
        }
      } else {
        if (mounted) setState(() => _selectedAddress = "No address found. Please add in Profile.");
      }
    } catch (e) {
      if (mounted) setState(() => _selectedAddress = "Error fetching address");
    }
  }

  void _changeAddress() {
    if (_userAddresses.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("No saved addresses. Add one in Profile!")));
      return;
    }
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ..._userAddresses.map((addr) {
              return ListTile(
                leading: const Icon(Icons.location_on, color: AppColors.primaryGreen),
                title: Text(addr['type'] ?? 'Address'),
                subtitle: Text(addr['full_address']),
                onTap: () {
                  setState(() {
                    _selectedAddress = addr['full_address'];
                    _selectedLat = addr['lat']?.toDouble();
                    _selectedLng = addr['lng']?.toDouble();
                  });
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    _buildPaymentMethodTile(PaymentMethod.card, "Credit/Debit Card", Icons.credit_card, "Pay securely via Safepay"),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          TextButton(onPressed: _changeAddress, child: const Text("Edit", style: TextStyle(color: AppColors.primaryGreen))),
        ],
      ),
    );
  }
  ...
  bool _validateOrder(UserModel user) {
    if (widget.cartItems.isEmpty) {
      _showError('Your cart is empty. Add items before ordering.');
      return false;
    }

    if (_selectedAddress.isEmpty ||
        _selectedAddress.contains('Loading') ||
        _selectedAddress.contains('Error') ||
        _selectedAddress.contains('Select') ||
        _selectedAddress == "No address found. Please add in Profile.") {
      _showError('Please select a valid delivery address.');
      return false;
    }

    final phone = user.phone?.trim() ?? '';
    if (phone.length != 11 || !phone.startsWith('03')) {
      _showError('Please add a valid Pakistani number (03XXXXXXXXX) in your profile.');
      return false;
    }
  ...
  Future<void> _handlePlaceOrder(UserModel user, OrderService orderService, PaymentService paymentService, CartModel cart, double total) async {
    ...
    try {
      ...
      for (var entry in vendorGroups.entries) {
        ...
        // Fetch Vendor Info for routing
        String? storeName;
        double? storeLat;
        double? storeLng;
        try {
          var vDoc = await FirebaseFirestore.instance.collection('users').doc(vendorId).get();
          if (vDoc.exists) {
            final vData = vDoc.data();
            if (vData != null) {
              storeName = (vData['business_name'] != null && vData['business_name'].toString().trim().isNotEmpty)
                  ? vData['business_name']
                  : (vData['full_name'] ?? 'Store');
              storeLat = vData['lat']?.toDouble();
              storeLng = vData['lng']?.toDouble();
            }
          }
        } catch (_) {}

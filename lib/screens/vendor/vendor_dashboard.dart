...
class _VendorDashboardState extends State<VendorDashboard> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (user.uid != null) {
        orderService.startOrderWatcher(user.uid!);
      }
    });
  ...
  void dispose() {
    try {
      final orderService = Provider.of<OrderService>(context, listen: false);
      orderService.stopOrderWatcher();
    } catch (e) {
      print("Error stopping order watcher: \$e");
    }
...
class _DashboardTab extends StatelessWidget {
  ...
  Widget _buildStatsGrid() {
    print("VendorDashboard: Fetching stats for Vendor UID: ${user.uid}");
...
class _InventoryTabState extends State<_InventoryTab> {
  final InventoryPredictor _predictor = InventoryPredictor();
  Map<String, double> _localPredictions = {};
  Map<String, double> _globalPredictions = {};
  ...
  Future<void> _fetchPredictions() async {
    ...
    try {
      final localPreds = await _predictor.predictDemand(widget.user.uid!);
      final globalPreds = await _predictor.getGlobalDemand();
      if (mounted) {
        setState(() {
          _localPredictions = localPreds;
          _globalPredictions = globalPreds;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF4F7F6),
      appBar: AppBar(
        title: const Text("Inventory & High Demand", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        actions: [
          if (_isLoadingPredictions)
            ...
            IconButton(
              ...
              tooltip: 'Refresh High Demand Products',
              onPressed: () {
      ...
      body: widget.user.uid == null
          ? const Center(child: Text("Please login"))
          : StreamBuilder<List<ProductModel>>(
              stream: widget.productService.getVendorProducts(widget.user.uid!),
              builder: (context, snapshot) {
                final products = snapshot.data ?? [];

                // Build stock map for restock alerts
                final stockMap = {for (var p in products) p.name: p.stockQuantity};

                // Match local predictions by product name (case-insensitive substring match)
                Map<String, double> _getPredictionForProduct(ProductModel p) {
                  // Try exact match first
                  if (_localPredictions.containsKey(p.name)) return {p.name: _localPredictions[p.name]!};
                  // Try case-insensitive match
                  for (final key in _localPredictions.keys) {
                    if (key.toLowerCase() == p.name.toLowerCase()) return {p.name: _localPredictions[key]!};
                  }
                  // Try partial match
                  for (final key in _localPredictions.keys) {
                    if (p.name.toLowerCase().contains(key.toLowerCase()) ||
                        key.toLowerCase().contains(p.name.toLowerCase())) {
                      return {p.name: _localPredictions[key]!};
                    }
                  }
                  return {};
                }
                  ...
                  slivers: [
                          ...
                          children: [
                            const Row(
                              children: [
                                ...
                                SizedBox(width: 8),
                                Text('High Demand Products', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 15, letterSpacing: 0.5)),
                              ],
                            ),
                            const SizedBox(height: 4),
                            Text(
                              _isLoadingPredictions
                                  ? 'Calculating high demand products...'
                                  : 'Showing top products ordered in the last 24 hours',
                              style: const TextStyle(color: Colors.white70, fontSize: 12),
                            ),
                            if (!_isLoadingPredictions && _globalPredictions.isNotEmpty) ...[
                              const SizedBox(height: 14),
                              SingleChildScrollView(
                                scrollDirection: Axis.horizontal,
                                child: Row(
                                  children: _globalPredictions.entries.take(5).map((e) {
                                    return Container(
                                      ...
                                      child: Column(
                                        children: [
                                          Text(e.key, style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)),
                                          const SizedBox(height: 2),
                                          Text('${e.value.toInt()} units', style: const TextStyle(color: Colors.white70, fontSize: 11)),
                                        ],
                                      ),
                                    );
                                  }).toList(),
                    ...
                    if (!_isLoadingPredictions)
                      FutureBuilder<List<String>>(
                        future: InventoryPredictor().generateRestockAlerts(stockMap, _localPredictions),
                                ...
                                children: [
                                  Row(
                                    children: [
                                      ...
                                      const SizedBox(width: 8),
                                      Text('${alerts.length} Restock Alert${alerts.length > 1 ? 's' : ''}', style: const TextStyle(fontWeight: FontWeight.w900, color: Colors.orange, fontSize: 13)),
                                    ],
                                  ),
                                  const SizedBox(height: 8),
                                  ...alerts.map((a) => Padding(
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        ...
                                        Expanded(child: Text(a, style: const TextStyle(fontSize: 11, color: Color(0xFF5D4037)))),
                                      ],
                                    ),
                                  )),
                                ],
                    ...
                    const SliverToBoxAdapter(
                      child: Padding(
                        ...
                        child: Text('Stock Levels & High Demand', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16)),
                      ),
                    ),
                    if (products.isEmpty)
                      const SliverFillRemaining(
                        child: Center(child: Text('No products yet. Tap + to add one.', style: TextStyle(color: Colors.grey))),
                      )
                    else
                      SliverList(
                        delegate: SliverChildBuilderDelegate(
                          (context, index) {
                            ...
                            final predMatch = _getPredictionForProduct(p);
                            final predictedDemand = predMatch.isNotEmpty ? predMatch.values.first : 0.0;
                            final isLowStock = p.stockQuantity < predictedDemand && predictedDemand > 0;
                            final isCritical = p.stockQuantity < 5;

                            Color urgencyColor = Colors.green;
                            String urgencyLabel = 'OK';
                            if (isCritical) { urgencyColor = Colors.red; urgencyLabel = 'CRITICAL'; }
                            else if (isLowStock) { urgencyColor = Colors.orange; urgencyLabel = 'LOW'; }
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    Container(
                                      ...
                                      child: ProductImage(imageUrl: p.imageUrl, borderRadius: BorderRadius.circular(12)),
                                    ),
                                    const SizedBox(width: 14),
                                    // Product info + ML prediction
                                    Expanded(
                                      child: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          Row(
                                            children: [
                                              Expanded(child: Text(p.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14), overflow: TextOverflow.ellipsis)),
                                              Container(
                                                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                                                decoration: BoxDecoration(color: urgencyColor.withOpacity(0.12), borderRadius: BorderRadius.circular(8)),
                                                child: Text(urgencyLabel, style: TextStyle(color: urgencyColor, fontSize: 10, fontWeight: FontWeight.w900)),
                                              ),
                                            ],
                                          ),
                                          ...
                                          Row(
                                            children: [
                                              ...
                                              Text('${p.stockQuantity} units', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: isCritical ? Colors.red : Colors.black87)),
                                            ],
                                          ),
                                          const SizedBox(height: 4),
                                          // ML Prediction row
                                          if (_isLoadingPredictions)
                                            ...
                                            Row(
                                              children: [
                                                const Icon(Icons.auto_awesome, size: 13, color: Color(0xFF1565C0)),
                                                const SizedBox(width: 4),
                                                Text('High Demand: ', style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                                                Text('${predictedDemand.toInt()} units ordered', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF1565C0))),
                                              ],
                                            )
                                          else
                                            Text('No demand data yet', style: TextStyle(fontSize: 11, color: Colors.grey.shade400, fontStyle: FontStyle.italic)),
                                        ],
                                      ),
                                    ),
                                    const SizedBox(width: 8),
                                    ...
                                    IconButton(
                                      ...
                                      onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => AddProductScreen(product: p))),
                                    ),
                                  ],
                                ),
                              ),

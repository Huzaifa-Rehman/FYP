...
class _AddressManagementScreenState extends State<AddressManagementScreen> {
              ...
              child: ElevatedButton(
                onPressed: () {
                  if (addressController.text.isNotEmpty) {
                    _saveAddress(uid, addressController.text, selectedType, selectedLoc);
  ...
  Future<void> _saveAddress(String uid, String address, String type, LatLng? loc) async {
    Map<String, dynamic> addrObj = {
      'full_address': address,
      'type': type,
    };
    if (loc != null) {
      addrObj['lat'] = loc.latitude;
      addrObj['lng'] = loc.longitude;
    }

    await _db.collection('users').doc(uid).update({
      'addresses': FieldValue.arrayUnion([addrObj])
    });
  }

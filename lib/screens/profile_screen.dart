...
class ProfileScreen extends StatelessWidget {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                        ...
                        children: [
                          Text(
                            isLoggedIn 
                              ...
                              : 'Guest User', 
                            style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Theme.of(context).textTheme.bodyLarge?.color)
                          ),
              ...
              Row(
                children: [
                  _actionCard(context, Icons.shopping_bag_outlined, 'Your Orders', AppColors.primaryGreen, const Color(0xFFE8F5E9), () {
                    Navigator.push(context, MaterialPageRoute(builder: (_) => const OrderHistoryScreen()));
                  }),
                  const SizedBox(width: 12),
                  _actionCard(context, Icons.account_balance_wallet_outlined, 'Grocer\nMoney', const Color(0xFFB97A4D), const Color(0xFFFFF3E0), () {
                    showDialog(context: context, builder: (_) => AlertDialog(
                      title: const Row(children: [Icon(Icons.account_balance_wallet, color: Color(0xFFB97A4D)), SizedBox(width: 8), Text('Grocer Money')]),
                      content: const Text('Your current wallet balance is Rs. 0.00\n\nTop up feature will be available soon.'),
                      actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Close'))],
                    ));
                  }),
                  const SizedBox(width: 12),
                  _actionCard(context, Icons.headset_mic_outlined, 'Need Help?', Colors.blue.shade700, Colors.blue.shade50, () {
                    showDialog(context: context, builder: (_) => AlertDialog(
                      title: const Row(children: [Icon(Icons.headset_mic, color: Colors.blue), SizedBox(width: 8), Text('Help Center')]),
                      content: const Text('Need assistance with your recent order?\n\nCall our 24/7 helpline at 111-222-333 or email support@speedygrocer.app'),
                      actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Call Now')), TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel'))],
                    ));
                  }),
                ],
              ),
                  ...
                  children: [
                    _settingsTile(Icons.palette_outlined, 'Appearance', trailingWidget: Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          Icon(Provider.of<ThemeProvider>(context).themeMode == ThemeMode.dark ? Icons.nightlight_round : Icons.wb_sunny_outlined, size: 14, color: AppColors.primaryGreen),
                          const SizedBox(width: 4),
                          Text(Provider.of<ThemeProvider>(context).themeMode == ThemeMode.dark ? 'DARK' : 'LIGHT', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: AppColors.primaryGreen)),
                        ],
                      ),
                    ), onTap: () {
                      ...
                      showDialog(context: context, builder: (_) => AlertDialog(
                        ...
                        content: Column(
                          ...
                          children: [
                            ListTile(
                              leading: const Icon(Icons.wb_sunny, color: AppColors.primaryGreen),
                              title: const Text('Light Mode', style: TextStyle(fontWeight: FontWeight.bold)),
                              trailing: themeProvider.themeMode == ThemeMode.light ? const Icon(Icons.check_circle, color: AppColors.primaryGreen) : null,
                              onTap: () {
                            ...
                            ListTile(
                              ...
                              trailing: themeProvider.themeMode == ThemeMode.dark ? const Icon(Icons.check_circle, color: AppColors.primaryGreen) : null,
                              onTap: () {
                            ...
                            ListTile(
                              ...
                              trailing: themeProvider.themeMode == ThemeMode.system ? const Icon(Icons.check_circle, color: AppColors.primaryGreen) : null,
                              onTap: () {
                ...
                child: Column(
                  children: [
                    ...
                    _settingsTile(Icons.info_outline, 'About Us', hasChevron: true, onTap: () {
                      showAboutDialog(
                        context: context,
                        applicationName: 'SpeedyGrocer',
                        applicationVersion: 'v4.2.0 (Build 902)',
                        applicationIcon: Container(
                          padding: const EdgeInsets.all(8),
                          decoration: BoxDecoration(color: AppColors.primaryGreen.withOpacity(0.1), shape: BoxShape.circle),
                          child: const Icon(Icons.shopping_bag, color: AppColors.primaryGreen, size: 40),
                        ),
                        children: [
                          const SizedBox(height: 16),
                          const Text('SpeedyGrocer is a lightning-fast grocery delivery app built as a Final Year Project.'),
                        ],
                      );
                    }),
                    const Divider(height: 1, indent: 50, endIndent: 16, color: Color(0xFFF0F2F5)),
  ...
  Widget _actionCard(BuildContext context, IconData icon, String label, Color iconColor, Color bgColor, VoidCallback onTap) {
    return Expanded(
      child: MouseRegion(
        cursor: SystemMouseCursors.click,
        child: GestureDetector(
          ...
          child: Container(
            ...
            child: Column(
              children: [
                ...
                Text(label, textAlign: TextAlign.center, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: Theme.of(context).textTheme.bodyLarge?.color, height: 1.2)),
              ],
            ),
          ),
        ),
  ...
  Widget _settingsTile(IconData icon, String title, {Widget? trailingWidget, bool hasChevron = false, Color? textColor, Color? iconColor, VoidCallback? onTap}) {
    return Builder(
      builder: (context) {
        return ListTile(
          onTap: onTap,
          leading: Icon(icon, color: iconColor ?? Theme.of(context).iconTheme.color, size: 24),
          title: Text(title, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: textColor ?? Theme.of(context).textTheme.bodyLarge?.color)),

  ...
  List<Map<String, dynamic>> _extractUniqueProducts(List<OrderModel> orders) {
    ...
    for (var order in orders) {
      for (var item in order.items) {
        ...
        // Keep the latest/first seen product details
        if (!products.containsKey(name)) {
          products[name] = item;
        }
        
        // Count frequency based on quantity in the order, or just frequency of appearance
        // If we want to count how many times they bought it total:
        final quantity = (item['quantity'] as num?)?.toInt() ?? 1;
        productFrequency[name] = (productFrequency[name] ?? 0) + quantity;
      }
    }

    // Sort the product names by frequency descending
    final sortedProductNames = productFrequency.keys.toList()
      ..sort((a, b) => productFrequency[b]!.compareTo(productFrequency[a]!));

    // Take top 3
    final top3Names = sortedProductNames.take(3).toList();

    return top3Names.map((name) => products[name]!).toList();
  }
              ...
              ? GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: const Text('ORDER AGAIN',
                        style: TextStyle(color: AppColors.primaryGreen, fontSize: 11, fontWeight: FontWeight.w800)),
                  ),
                )
              : Container(
                  decoration: BoxDecoration(

      ...
      slivers: [
        ...
        SliverPadding(
          ...
          sliver: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance.collection('categories').where('type', isEqualTo: 'grocery').snapshots(),
              ...
              return SliverGrid(
                ...
                delegate: SliverChildBuilderDelegate(
                  (context, index) {
                    ...
                    return _CategoryTile(category: catData);
                  },
        ...
        SliverPadding(
          ...
          sliver: StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance.collection('categories').where('type', isEqualTo: 'snack').snapshots(),
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) return const SliverToBoxAdapter(child: Center(child: CircularProgressIndicator()));
              final categories = snapshot.data?.docs ?? [];
              if (categories.isEmpty) return const SliverToBoxAdapter(child: SizedBox());
